...
export const getAssociatedTokenAccount = async (
  ...
  try {
    logger.info(
      `[getAssociatedTokenAccount] Fetching associated token account for address: ${addr.toBase58()} and token mint: ${tokenMintAddress.toBase58()}`,
    );
    ...
    logger.info(
      `[getAssociatedTokenAccount] Found associated token account: ${account.toBase58()}`,
    );
    return account;
  } catch (error) {
    logger.error(`[getAssociatedTokenAccount] Error fetching associated token account: ${error}`);
    throw new GenericError(
      "Failed to fetch associated token account.",
...
export const tokenAccounts = async (
  ...
  try {
    ...
    if (!userTokenAccount) {
      throw new GenericError(
        "Failed to get associated token account for user.",
        StatusCodes.INTERNAL_SERVER_ERROR,
      );
    }
    ...
    throw error instanceof GenericError
      ? error
      : new GenericError("Failed to retrieve token accounts.", StatusCodes.INTERNAL_SERVER_ERROR);
  }
};

export const getTxObject = async (data: IGetTxObject): Promise<ResultWithError> => {
  try {
...
export const parseToPrecision = (amount: number, decimals: number) => {
  try {
    logger.info(
      `[parseToPrecision] Parsing amount to precision: ${amount} with decimals: ${decimals}`,
    );
    ...
    throw new GenericError(
      "Failed to parse amount to precision.",
      StatusCodes.INTERNAL_SERVER_ERROR,
    );
  }
};
